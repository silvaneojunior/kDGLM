...
```{r, results='hide'}
plot(coef(fitted.data), "atanh.rho", plot.pkg = "base")
```
...
```{r, results='hide'}
plot(coef(fitted.data), plot.pkg = "base")$plot
```
...
```{r, results='hide'}
plot(coef(fitted.data), "Var.Poly.Level", plot.pkg = "base")$plot
```
...
Similar to the `plot_lat_var` function, the user can also specify the name of the linear predictor to be plotted by the `plot_lin_pred` function (although, in this example, there is only one linear predictor).
...
In this example we model number of hospital admissions from gastroenteritis in Brazil from 2010 to 2022. Our package provides the `gatroBR` dataset with the pertinent data for our models, which includes:
 ...
 - Population: The estimated population of that of state and date (to be used as a offset).

Supplementary infomations can be found in the documentation (see `help(gastroBR)`).
...
Notice that theres a consistent trend of (log) linear decay of the rate of hosptial admissions over time, until April of 2020, when there a abrubt reduction of hospital admission duo to the pandemic of COVID-19, which is then followed by what seems to be a return to the previous level (more observations would be necessary to be sure). We can also note that the data has a clear seasonal pattern, which have a period of 12 months (1 year).
...
```{r echo=TRUE}
structure <- polynomial_block(rate = 1, order = 2, D = c(0.95, 0.975))
```
...
plot(fitted.model, plot.pkg = "base") |> save.fig(file = "vignettes/plot17.pdf")
```

Clearly the model described above is too simply to explain the data, in particular, it does not take into account any form of seasonal pattern. Let us proceed then by adding a harmonic block:

```{r echo=TRUE}
structure <- polynomial_block(rate = 1, order = 2, D = c(0.95, 0.975)) +
  harmonic_block(rate = 1, period = 12, D = 0.98)
```
...
Notice that this change significantly improves all metrics provided by the model summary, which indicates that we are going in the right direction. We leave as an exercise for the user to test diffent order for the harmonic block.

The previous model could capture the mean behavior of the series reasonably well, still, two deficiencies of that model standout: First, the overconfidence in the predictions, evidenced by the particularly thin credibility interval; And second, the difficulty the model has to adapt to the pandemic period.

The first problem comes from the fact that we are using a Poisson model, which implies that $Var[Y_t|\eta_t]=\mathbb{E}[Y_t|\eta_t]$, which means that $Var[Y_t]=\mathbb{E}[Var[Y_t|\eta_t]]+Var[\mathbb{E}[Y_t|\eta_t]]=\mathbb{E}[\eta_t]+Var[\eta_t]$, but we expect that $Var[\eta_t]$ would be relatively small for latter observations, as such, the variance of $Y_t$ should be very close to its mean after a reasonable amount of observations. In this scenario, the coefficient of variation, defined as $\frac{\sqrt{Var[Y_t]}}{\mathbb{E}[Y_t]}$ goes to $0$ as $\mathbb{E}[Y_t]$ grows, in particular, for data in the scale we are dealing in this particular problem, we would expect a very low coefficient of variation if the Poisson model were adequate, but that is not what we observe. This phenomena is called *overdipersion* and is a well known problem in literature. To solve it, we can add an additional block representing a white noise that is added to the linear predictor at each time, but does not affect previous or future observation, so as to capture the overdipersion:
...
For the second problem, that of slow adaptation after the start of the pandemic, the ideal approach would be to make an intervation, increasing the uncertainty about the latent states at the beggining of the pandemic period and allowing our model to quickly adapt to the new scenario. This approach is ideal because we expect a change of behavior during the pandemic even before observing the data. Still, for didactic purposes, we will first present how the automated monitoring can also be used to solve this same problem, this approach is ideal for problems were we **do not** known **when** or if a change of behavior happened.

```{r echo=TRUE}
structure <- polynomial_block(
  rate = 1, order = 2, D = c(0.95, 0.975),
  monitoring = c(TRUE, TRUE)
) +
  harmonic_block(rate = 1, period = 12, D = 0.98) +
  noise_block(rate = 1)
```
...
summary(fitted.model)
plot(fitted.model, plot.pkg = "base")$plot
plot(fitted.model, lag = -1, plot.pkg = "base")$plot
```
...
One aspect of the model that may bother the reader is the excendently high uncertanty at the first observations. This behavior is duo to our approach to the estimation of the variance of the white noise introduced by the `noise_block` function (see the associated documentation for details), specifically, the initial steps can be a bit too sensitive to bad prior for the white noise. As such, we highly recommend the user to do a sensibility analys to choose this parameter:

```{r, results='hide'}
structure <- polynomial_block(rate = 1, order = 2, D = c(0.95, 0.975)) +
  harmonic_block(rate = 1, period = 12, D = 0.98) +
  noise_block(rate = 1, R1 = "H") # Setting the initial variance as a unknown parameter
...
Notice that, this time around, we chose to  make an intervention at the begining of the pandemic, instead of an automated approach. As mentioned before, this approach is preferable in this scenario, since we were aware that the pandemic would affect our outcome.

Again, the new changes improve the comparison metrics even further, leading to the conclusion that our last model is the best among those presented until now. We highly encourage the reader to run this example and experiment with some of the options our package offers, but that were not explored, such as changing the discount factors used in each block, the order of the block, adding/removing structural components, etc..

As a last side note, the user may not like the approach of chosing a specific value for the initial variance of the white noise introduced by the `noise_block`. Indeed, one may wish to define a prior distribution for this parameter and estimate it along with the other parameters. While we will not detail this approach for the sake of brevity, we would like to point out that or package does offers tools to facilitate this procedure:
...
First, for this model, we need the geographic information about Brazil, as such, we will use some auxiliary packages, namely \code{geobr}, \code{tidyverse}, \code{sf} and \code{spdep}, although the \code{kDGLM} package does not depend on them:
...
CAR.structure <- polynomial_block(rate = 1, D = 0.95, name = "CAR") |>
  block_mult(27) |>
  block_rename(levels(gastroBR$UF)) |>
  CAR_prior(scale = 100, rho = 1, adj.matrix = adj.matrix)

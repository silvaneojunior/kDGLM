...
- D:
  ...
  dimensions should be (2n) x (2n) x t, where n is the order of the
  harmonic block and t is the length of the outcomes. If D is a matrix,
  its dimensions should be (2n) x (2n) and the same discount matrix will
  be used in all observations. If D is a vector, it should have size t
  ...
  will be used for all latent states at all times.
...
- h:
  ...
  time). If a matrix, its dimension should be (2n) x t, where n is the
  order of the harmonic_block and t is the length of the series. If a
...
- H:
  ...
  should be (2n) x (2n) x t, where n is the order of the harmonic block
  and t is the length of the series. If H is a matrix, its dimensions
  should be (2n) x (2n) and its values will be used for each time. If H
  is a vector or scalar, a discount factor matrix will be created as a
...
- a1:
  ...
  to two times the order of the harmonic block. If a1 is a scalar, its
  value will be used for all latent states.
...
- R1:
  ...
  dimensions should be (2n) x (2n). If R1 is a vector or scalar, a
...
- monitoring:
  ...
  monitored (if automated monitoring is used). Its size should be 2n.
  The default is that only the first order component of this structure
  should be monitored.
...
For the details about the modelling of seasonal trends using harmonics
in the context of DLM's, see West and Harrison (1997) , chapter 8.
